/*
 * Site bootstrap script. Loaded from /boot.js via <Script src="/boot.js">
 * in src/app/layout.tsx with strategy="afterInteractive".
 *
 * Filename is deliberately neutral (per Raptive's Next.js guidance) so
 * common ad-blocker filter lists don't drop the request based on the
 * filename. Do not rename to ads.js / raptive-ads.js / adthrive.js etc.
 *
 * Contents (in order):
 *   1. Raptive main ad loader (creates ads.min.js script)
 *   2. Raptive ad-block recovery (ABR) — detects blocked requests and
 *      substitutes via fallback domains (html-load.com / content-loader.com)
 *
 * If you need to update either block, request the latest from Raptive
 * support — both blocks are provided verbatim and the recovery code is
 * intentionally obfuscated; do not hand-edit.
 */

/* ============================================================
 * 1. Raptive Head Tag (main ad loader)
 * ============================================================ */
(function (w, d) {
  ...
  s.src =
    'https://' +
    w.adthrive.host +
    '/sites/68f28aad3e1aa05cf10f4d54/ads.min.js?referrer=' +
...
/* ============================================================
 * 2. Ad-Block Recovery — primary detection (abd.js)
 * ============================================================ */
!(function (e) {
  const t = '__adblocker';
  if (-1 === e.cookie.indexOf(t)) {
    const n = new XMLHttpRequest();
    n.open('GET', 'https://ads.adthrive.com/abd/abd.js', !0);
    n.onreadystatechange = function () {
      if (XMLHttpRequest.DONE === n.readyState)
        if (200 === n.status) {
          ...
          e.cookie = t + '=true; expires=' + n.toUTCString() + '; path=/';
        }
    };
    n.send();
  }
...
/* ============================================================
 * 3. Ad-Block Recovery — light/essential fallback substitution
 * ============================================================
 * Cookie-gated: only fires if abd.js set the __adblocker cookie.
 * recoveryMode defaults to "light" (no data-abr-mode attribute set).
 */
(function () {
  var recoveryMode = 'light';
  ...
  function init() {
    ...
    if (detectionCookie === 'true') {
      ...
      // Poll for cookie to be set (max 5 seconds at 50ms intervals)
      var counter = 0;
