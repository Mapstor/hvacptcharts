...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const { title, description } = buildRefrigerantMetadata({
    ...
    maxTempF,
    pressure70F: p70,
    // hasPtData true when either the CoolProp chart or the datasheet-native
    // pressure-indexed table has content. Wave 1.6b routes r-448a and
    // r-438a here via the ptTable path.
    ...
    // datasheet-table product.
    primaryDatasheet: r.primaryDatasheet ?? null,
...
export default async function RefrigerantPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // H1 tracks the metadata branch: PT data present (either 1°F CoolProp
  // chart OR datasheet-native pressure-indexed ptTable) → "PT Chart";
  // no data → "Refrigerant Reference". Keeps the on-page heading honest
  // with the SERP surface generated by buildRefrigerantMetadata.
  const seo = seoName(r.displayName);
  const common = COMMON_NAME[slug];
  const prefix = common ? `${seo} (${common})` : seo;
  const hasAnyPt = r.ptChart.length > 0 || (r.ptTable !== undefined && r.ptTable.length > 0);
  const heading = hasAnyPt ? "PT Chart" : "Refrigerant Reference";
  const pageH1 = `${prefix} ${heading}`;
      ...
      <article className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
        ...
        {r.ptChart.length > 0 ? (
          ...
          <Section
            id="pt-chart"
            icon={<Gauge className="h-5 w-5 text-blue-600 dark:text-blue-400" />}
            title="Saturation pressure-temperature — datasheet table"
            number="01"
          >
            <PTDatasheetTable
              displayName={r.displayName}
              rows={r.ptTable}
              primaryDatasheet={r.primaryDatasheet}
            />
          </Section>
        ) : (
          <Section
