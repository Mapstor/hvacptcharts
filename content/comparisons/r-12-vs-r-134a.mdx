...
retrofitNotes: |
  R-12 → R-134a is the classic HFC retrofit and one of the most well-documented conversion paths in HVAC history. SAE J1661 is the governing automotive-industry standard for the R-12 → R-134a retrofit procedure — the procedural discipline below tracks its requirements, and manufacturer service bulletins for specific vehicles generally align with it. The substantive changes: oil, seals, filter-drier, and charge amount. Pressures and safety class stay the same.
  ...
  **Service ports and retrofit labeling.** Automotive R-134a uses different service port fittings than R-12 (SAE J639/J2196) — larger diameter, different thread. Retrofit kits include the port adapters. This physical difference is intentional: prevents accidental cross-contamination between R-12 and R-134a supply. Retrofitted systems also require a permanent label identifying the refrigerant now in the system — a standard SAE / EPA retrofit-procedure requirement that keeps the next technician from cross-charging it back to R-12 or adding the wrong oil.
